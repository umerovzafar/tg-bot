final translations = {
  'choose_language': {
    'ru': '🌐 Пожалуйста, выбери язык:',
    'uz': '🌐 Iltimos, tilni tanlang:',
  },
  'send_phone': {
    'ru': '📞 Пожалуйста, отправьте свой номер телефона:',
    'uz': '📞 Iltimos, telefon raqamingizni yuboring:',
  },
  'send_name': {
    'ru': '📝 Пожалуйста, введите ваше полное имя (ФИО):',
    'uz': '📝 Iltimos, to‘liq ismingizni kiriting (F.I.Sh):',
  },
  'data_saved': {
    'ru': '✅ Спасибо! Ваши данные сохранены.',
    'uz': '✅ Rahmat! Maʼlumotlaringiz saqlandi.',
  },
  'registration_complete': {
    'ru': '🎉 Вы успешно прошли регистрацию!',
    'uz': '🎉 Roʻyxatdan muvaffaqiyatli oʻtdingiz!',
  },
  'menu': {'ru': '📋 Главное меню:', 'uz': '📋 Asosiy menyu:'},
  'payment': {'ru': '💳 Оплата', 'uz': '💳 Toʻlov'},
  'consultation': {'ru': '💬 Консультация', 'uz': '💬 Konsultatsiya'},
  'profile': {'ru': '👤 Профиль', 'uz': '👤 Profil'},
  'send_contact_button': {
    'ru': '📲 Отправить номер',
    'uz': '📲 Raqamni yuborish',
  },
  'cancel': {'ru': '❌ Отмена', 'uz': '❌ Bekor qilish'},
  'profile_info': {
    'ru':
        '👤 Ваш профиль:\n\n📛 ФИО: {full_name}\n📞 Телефон: {phone}\n🌐 Язык: {language}',
    'uz':
        '👤 Profilingiz:\n\n📛 F.I.Sh: {full_name}\n📞 Telefon: {phone}\n🌐 Til: {language}',
  },
  'consultation_info': {
    'ru':
        '💬 Для консультации, пожалуйста, обратитесь по номеру:\n\n📞 +998998495199\n\nНаши специалисты с радостью вам помогут!',
    'uz':
        '💬 Konsultatsiya uchun quyidagi telefon raqamiga murojaat qilishingiz mumkin:\n\n📞 +998953272721\n\nMutaxassislarimiz sizga mamnuniyat bilan yordam beradi!',
  },
  'payment_menu': {
    'ru': '💳 Выберите способ оплаты:',
    'uz': '💳 Toʻlov usulini tanlang:',
  },
  'card_payment': {'ru': 'Перевод на карту', 'uz': 'Kartaga oʻtkazish'},
  'installment_payment': {
    'ru': 'Оплата в рассрочку',
    'uz': 'Boʻlib-boʻlib toʻlov',
  },
  'back': {'ru': '🔙 Назад', 'uz': '🔙 Orqaga'},
  'installment_payment_info': {
    'ru':
        '💳 Вы выбрали способ оплаты в рассрочку. Для оформления перейдите по следующей ссылке:\n\nhttps://n.2u.uz/H7hfbo',
    'uz':
        '💳 Siz boʻlib-boʻlib toʻlov usulini tanladingiz. Roʻyxatdan oʻtish uchun quyidagi havolaga oʻting:\n\nhttps://n.2u.uz/H7hfbo',
  },
  'card_payment_info': {
    'ru':
        '✅ Вы выбрали способ оплаты: Перевод на карту.\n\n📝 Пожалуйста, переведите сумму на следующий номер карты:\n\n💳 Номер карты: 5614 6815 1883 8507\n👤 Получатель: Рахматова Анастасия\n\nПосле перевода, пожалуйста, подтвердите!',
    'uz':
        '✅ Siz toʻlov usulini tanladingiz: Kartaga oʻtkazish.\n\n📝 Iltimos, summa quyidagi karta raqamiga oʻtkazing:\n\n💳 Karta raqami: 5614 6815 1883 8507\n👤 Oʻtkazuvchi: Raxmatova Anastasiya\n\nPul oʻtkazmasini amalga oshirganingizdan soʻng, iltimos tasdiqlang!',
  },
};
